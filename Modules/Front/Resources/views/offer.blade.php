    ...
    <div class="offers-container">
      <div class="horizontal-inner-container offer-container">
          <ol class="breadcrumb">
            ...
            <li><a href="{{route('search')}}?job[]={{$offer->job_1}}">{{ strtoupper(Modules\Extranet\Entities\Tools\SiteList::getListValue($offer->job_1, 'jobs1')) }}</a></li>
            <li>{{$offer->title}}</li>
          </ol>
          <h1>{{$offer->title}}</h1>
          <div class="separator"></div>
          <p class="first-info">{{$offer->address}}, Contrat {{ Modules\Extranet\Entities\Tools\SiteList::getListValue($offer->contract, 'contracts') }} - Publié le {{ Date('d/m/Y', $offer->start_at )}} </p>
          <div class="col-sm-4 col-md-3 information">
            ...
            <div class="block-info">
              <p><b>Contrat:</b></p>
              <p>{{ Modules\Extranet\Entities\Tools\SiteList::getListValue($offer->contract, 'contracts') }}</p>
            </div>
            <div class="block-info">
              <p><b>À partir du:</b></p>
              <p>{{ Date('d/m/Y', $offer->start_at )}}</p>
						...
						<div class="block-info">
							<p><b>Métier:</b></p>
							<p>{{ Modules\Extranet\Entities\Tools\SiteList::getListValue($offer->job_1, 'jobs1') }}</p>
						</div>
						@endif
						@if($offer->job_2)
            <div class="block-info">
              <p><b>Secteur:</b></p>
              <p>{{ Modules\Extranet\Entities\Tools\SiteList::getListValue($offer->job_2, 'jobs1') }} </p>
            </div>
						@endif
						@if($offer->salary)
	            <div class="block-info">
	              <p><b>Salaire:</b></p>
	              <p>{{ Modules\Extranet\Entities\Tools\SiteList::getListValue($offer->salary, 'salaries') }}</p>
	            </div>
						@endif
            ...
            <div class="share-container">
              @php
               ...
               <a href="https://www.facebook.com/sharer/sharer.php?u={{$shareUrl}}&t={{$title}}"
        					...
        					<img src="{{asset('modules/front/images/fb_icon.jpg')}}" class="social-icon">
        				</a>
                ...
                <a href="https://twitter.com/share?url={{$shareUrl}}&text={{$title}}"
        					...
        					<img src="{{asset('modules/front/images/tw_icon.jpg')}}" class="social-icon">
        				</a>

                <a href="mailto:?subject={{$title}}&body={{$shareUrl}}"
        					class="mail-button">
        					<img src="{{asset('modules/front/images/mail_icon.jpg')}}" class="social-icon">
        				</a>
          ...
          <div class="col-sm-8 col-md-9 description">
            ...
            <p><b>Logiciel: </b>PACK OFFICE</p>
						<p><b>Horaries: </b>
							@php
								$values = Modules\Extranet\Entities\Tools\SiteList::getListValue($offer->schedule, 'schedule');
							@endphp
