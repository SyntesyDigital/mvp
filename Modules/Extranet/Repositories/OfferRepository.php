...
use Modules\Extranet\Entities\Offers\Tags;
use Auth;
use Datatables;
use DB;
...
class OfferRepository extends BaseRepository
{
    ...
    {
        return "Modules\\Extranet\\Entities\\Offers\Offer";
    ...
    public function getDataTableData()
    {
        ...
        return Datatables::eloquent($offers)
            ...
            ->addColumn('applied', function ($item) {
                return '<a href="'.route('extranet.admin.offer.applications.show', $item).'" class="btn btn-link"> <i class="fa fa-address-card"></i> '.$item->applications()->count().' </a>';
            })

            ->addColumn('action', function ($item) {
                $html = '';

                $html .= '&nbsp; <a title="'.Lang::get("architect::datatables.edit").'"  href="'.route('extranet.admin.offers.show', $item).'" class="btn btn-link"><i class="fa fa-pencil"></i> </a>';

                if (Auth::user()->hasRole('admin') || (Auth::user()->hasRole('recruiter') && $item->recipient_id == Auth::user()->id)) {
                    $html .= '&nbsp; <a  title="'.Lang::get("architect::datatables.delete").'" href="'.route('extranet.admin.offers.delete', $item).'" data-ajax="'.route('extranet.admin.offers.delete', $item).'" data-toogle="delete" data-confirm-message="Êtes-vous sur de vouloir supprimer cette offre ?" class="btn btn-link text-danger" ><i class="fa fa-trash"></i> </a>';
                }
