...
@section('content')
    ...
    {!!
        Form::open([
            'url' => isset($user)
                ? route('extranet.admin.candidates.update', $user)
                : route('extranet.admin.candidates.store'),
            'method' => 'POST',
    ...
    !!}
        ...
        {!!
            Form::hidden('type', isset($user->candidate->type) && $user->candidate->type != '' ? $user->candidate->type : Modules\Extranet\Entities\Offers\Candidate::TYPE_NORMAL, [
                'id' => 'type',
            ])
        !!}
          ...
          <div class="container">
            <div class="row">
              <div class="col-md-12">
                ...
                <a href="{{route('extranet.admin.candidates.index')}}" class="btn btn-default"> <i class="fa fa-angle-left"></i> </a>
                ...
                <div class="float-buttons pull-right">
                  ...
                  <div class="actions-dropdown">
                      ...
                      <ul class="dropdown-menu dropdown-menu-right default-padding">
                          ...
                          @if(isset($user))
                          <li>
                              <a href="#" data-redirect="{{route('extranet.admin.candidates.index')}}" data-ajax="{{ route('extranet.admin.candidates.delete', $user) }}" data-confirm-message="{{Lang::get('architect::datatables.sure')}}" data-toogle="delete" class="text-danger">
        ...
        <div class="container rightbar-page candidate-page">
            ...
            <div class="col-md-9 page-content  field-group">
              ...
              <div id="headingtitle" class="btn btn-link" data-toggle="collapse" data-target="#collapsetitle" aria-expanded="true" aria-controls="collapsetitle">
                @if($user->candidate->type == Modules\Extranet\Entities\Offers\Candidate::TYPE_INTERIM)
                  ...
                  <div class="row">
                      <div class="col-md-12">
                          <div class="form-group {{$errors->has("civility") ? 'has-error' : ''}}">
                              ...
                              <div class="radio">
                                  <label>
                                      @if(isset($user) && $user->candidate->civility == Modules\Extranet\Entities\Offers\Candidate::CIVILITY_FEMALE )
                                        @php
                                          $arrayM = [];
                                      ...
                                      @endif
                                      {{
                                          Form::radio('civility', Modules\Extranet\Entities\Offers\Candidate::CIVILITY_MALE,$arrayM)
                                      }}
                                      Monsieur
                                  </label>
                                  ...
                                  <label>
                                      {{
                                          Form::radio('civility', Modules\Extranet\Entities\Offers\Candidate::CIVILITY_FEMALE,$arrayF)
                                      }}
                                      Madame
                                  </label>
                              </div>
              ...
              <div id="collapserecherche" class="collapse in" aria-labelledby="headingrecherche" aria-expanded="true" aria-controls="collapserecherche" style="">
                  <div class="field-form">
                      ...
                      <div class="form-group">
                          ...
                          @php
            								$list = Modules\Extranet\Entities\Tools\SiteList::where('identifier', 'contracts')->first();
                    ...
                    <div class="field-form">
                        <div id="candidate-documents"
                          config="{{ base64_encode(json_encode([
                              'type' => 'ajax',
                              'route' => route('extranet.admin.candidates.documents.data',$user->candidate()->first())
                          ], true))}}"
                        ></div>
                    </div>
                </div>
          ...
          <div class="sidebar">
              ...
              @if(isset($user))
                  ...
                  {!!
                      Form::select(
                          'tags[]',
                          \Modules\Extranet\Entities\Tag::pluck('name', 'id'),
                  ...
                  {!!
                      Form::open([
                          'url' => route('extranet.admin.candidates.updatetags', $user),
                          'method' => 'POST'
                          ])
                  !!}
...
@push('javascripts-libs')
    ...
    <!-- Toastr -->
    {{ Html::style('/modules/extranet/plugins/toastr/toastr.min.css') }}
    {{ Html::script('/modules/extranet/plugins/toastr/toastr.min.js') }}
...
@push('javascripts')
    ...
    {{ Html::script('/modules/extranet/js/admin/users/candidatesform.js') }}


    <script>
        var routes = {
            data : '{{ isset($user) ? route("extranet.admin.candidates.applications.data",  $user) : null }}',
            uploads : {
                resumeFile : '{{ route('extranet.admin.candidates.filestore') }}',
                recommendationLetterFile : '{{ route('extranet.admin.candidates.filestore') }}',
             }
        };

        var inactive_value = '{{ App\Models\User::STATUS_INACTIVE }}';
        var type_interim_value = '{{ Modules\Extranet\Entities\Offers\Candidate::TYPE_INTERIM }}';
